...
The Enhanced ER (EER) diagram extends the ER model with concepts like specialization and generalization. In our model, a `User` entity could be generalized from `Patient`, `Doctor`, and `Admin` entities.
...
Here, we used specialization to model different types of users. For example, `Doctor` and `Patient` are specializations of a more general `Person` or `User` entity, inheriting common attributes like name and contact information while also having their own specific attributes.
